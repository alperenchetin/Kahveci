      ...
      {!loading && (
        <>
          <div className="d-flex justify-content-between align-items-center">
            <h3 className="text-success">Sipariş Özeti</h3>
            <span className={`btn btn-outline-${badgeTypeColor} fs-6`}>
              {data.status}
            </span>
          </div>
          <div className="mt-3">
            <div className="border py-3 px-2">İsim : {userInput.name}</div>
            <div className="border py-3 px-2">Email : {userInput.email}</div>
            <div className="border py-3 px-2">
              Telefon : {userInput.phoneNumber}
            </div>
            <div className="border py-3 px-2">
              <h4 className="text-success">Ürünler</h4>
              <div className="p-3">
                {data.cartItems?.map(
                    ...
                    return (
                      <div className="d-flex" key={index}>
                        <div className="d-flex w-100 justify-content-between">
                          ...
                          <p>
                            {cartItem.menuItem?.price} x {cartItem.quantity} = TL
                          </p>
                        </div>
                        <p style={{ width: "70px", textAlign: "right" }}>
                          
                          {(cartItem.menuItem?.price ?? 0) *
                            (cartItem.quantity ?? 0)} TL
                        </p>
                      </div>
                    );
                  }
